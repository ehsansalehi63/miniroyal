  ...
  return (
    <article className="group relative min-w-0 bg-white">
      <div className="relative aspect-[4/5] overflow-hidden bg-[#f2eee9]">
        ...
        <div className="absolute inset-x-3 top-3 flex items-start justify-between"><div className="flex gap-1">{discount > 0 && <span className="bg-white px-2.5 py-1 text-[10px] font-black text-stone-950">-{toPersianDigits(discount)}٪</span>}{product.isSpecialOffer && <span className="bg-amber-300 px-2.5 py-1 text-[10px] font-black text-stone-950">پیشنهاد</span>}</div><button aria-label="افزودن به علاقه‌مندی‌ها" className="grid size-9 place-items-center rounded-full bg-white/90 text-stone-800 transition hover:bg-stone-950 hover:text-white"><Heart className="size-4" /></button></div>
        <Link href={`/product/${product.slug}?tryon=true`} className="absolute bottom-3 right-3 flex items-center gap-1.5 bg-stone-950/90 px-3 py-2 text-[10px] font-black text-white opacity-0 transition group-hover:opacity-100"><Sparkles className="size-3.5 text-amber-300" />پرو آنلاین</Link>
      </div>
      <div className="px-1 py-4" dir="rtl">
        ...
        <div className="mt-3 border-t border-stone-100 pt-3"><div className="flex items-end justify-between"><div>{product.salePrice && <span className="block text-[10px] text-stone-400 line-through">{formatToman(product.basePrice)}</span>}<span className="text-sm font-black text-stone-950">{formatToman(price)}</span></div><Link href={`/product/${product.slug}`} className="text-[11px] font-black text-violet-700">مشاهده ←</Link></div><Link href={`/product/${product.slug}?tryon=true#tryon-section`} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-violet-200 bg-violet-50 py-2.5 text-[11px] font-black text-violet-800 transition hover:border-violet-700 hover:bg-violet-700 hover:text-white"><Sparkles className="size-3.5" /> پرو آنلاین همین لباس</Link></div>
      </div>
    </article>
  );
}
