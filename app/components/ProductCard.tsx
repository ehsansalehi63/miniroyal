  ...
  return (
    <article className="group relative min-w-0 bg-white">
      ...
      <div className="px-1 py-4" dir="rtl">
        ...
        <div className="mt-3 flex items-end justify-between border-t border-stone-100 pt-3"><div>{product.salePrice && <span className="block text-[10px] text-stone-400 line-through">{formatToman(product.basePrice)}</span>}<span className="text-sm font-black text-stone-950">{formatToman(price)}</span></div><Link href={`/product/${product.slug}`} className="text-[11px] font-black text-violet-700">مشاهده ←</Link></div>
      </div>
    </article>
  );
}
