  ...
  return (
    <article className="group relative min-w-0 bg-white">
      ...
      <div className="px-1 py-4" dir="rtl">
        ...
        <div className="mt-3 border-t border-stone-100 pt-3"><div className="flex items-end justify-between gap-2">{/* قیمت قبلی با اندازهٔ ۱۰px و رنگ stone-400 عملاً ناخوانا بود؛ کنتراست و اندازه کمی بیشتر شد. */}<div className="min-w-0">{product.salePrice && <span className="block text-[11px] font-semibold text-stone-500 line-through decoration-stone-400">{formatToman(product.basePrice)}</span>}<span className="mt-0.5 block text-sm font-black text-stone-950">{formatToman(price)}</span></div><Link href={`/product/${product.slug}`} className="shrink-0 text-[11px] font-black text-violet-700 hover:text-violet-900">مشاهده ←</Link></div>
