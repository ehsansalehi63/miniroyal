  ...
  return (
    <header className="sticky top-0 z-50 border-b border-stone-200/80 bg-[#fffaf3]/95 backdrop-blur-xl">
      ...
      <div className="mx-auto flex site-container items-center gap-3 px-4 py-4 lg:gap-8">
        ...
        <Link href="/" className="group flex shrink-0 items-center gap-3"><span className="grid size-11 overflow-hidden rounded-full border border-amber-400 bg-stone-950 shadow-lg transition group-hover:rotate-6"><img src="/images/brand/miniroyal-logo.webp" width={48} height={72} alt="لوگوی مینی رویال" className="size-full object-cover" /></span><span className="hidden leading-none sm:block"><span className="block text-xl font-black tracking-tight text-stone-950">مینی رویال</span><span className="mt-1 block text-[10px] font-bold tracking-[0.18em] text-violet-700">KIDS COUTURE</span></span></Link>
        ...
        <div className="flex items-center gap-1.5">{customer ? <Link href="/account" className="hidden items-center gap-1 rounded-full border border-violet-300 px-3 py-2.5 text-[11px] font-black text-violet-800 transition hover:bg-violet-50 sm:flex" aria-label="ورود به کارتابل کاربر"><User className="size-3.5" /><span className="max-w-28 truncate">سلام {customer.fullName}</span></Link> : <><Link href="/account?mode=login" className="hidden items-center gap-1 rounded-full border border-violet-300 px-3 py-2.5 text-[11px] font-black text-violet-800 transition hover:bg-violet-50 sm:flex"><User className="size-3.5" /> ورود</Link><Link href="/account?mode=register" className="hidden rounded-full bg-amber-300 px-3 py-2.5 text-[11px] font-black text-stone-950 transition hover:bg-amber-200 sm:flex">عضویت</Link></>}<Link href="/virtual-tryon" className="hidden items-center gap-2 rounded-full bg-gradient-to-l from-violet-700 to-fuchsia-600 px-4 py-2.5 text-[11px] font-black text-white shadow-lg shadow-violet-200 ring-2 ring-violet-200 transition hover:-translate-y-0.5 hover:from-violet-800 hover:to-fuchsia-700 md:flex"><Sparkles className="size-4" /> پرو آنلاین سایز</Link><Link href="/cart" className="relative grid size-11 place-items-center rounded-full bg-violet-700 text-white shadow-lg shadow-violet-200 transition hover:bg-violet-800" aria-label="سبد خرید"><ShoppingBag className="size-5" /><span className="absolute -right-1 -top-1 grid size-5 place-items-center rounded-full bg-amber-400 text-[10px] font-black text-stone-950">{toPersianDigits(totalItems)}</span></Link></div>
      </div>
      <nav className="hidden border-t border-stone-200/70 bg-white/60 lg:block"><div className="mx-auto flex site-container items-center gap-1 px-4"><Link href="/" className="px-4 py-3 text-xs font-black text-stone-900 hover:text-violet-700">خانه</Link><Link href="/shop" className="px-4 py-3 text-xs font-black text-stone-900 hover:text-violet-700">فروشگاه</Link><div className="relative" onMouseEnter={() => setCategoryOpen(true)} onMouseLeave={() => setCategoryOpen(false)}><button onClick={() => setCategoryOpen((value) => !value)} className="flex items-center gap-1 px-4 py-3 text-xs font-black text-stone-900 hover:text-violet-700">همه دسته‌بندی‌ها <ChevronDown className="size-3.5" /></button>{categoryOpen && <div className="absolute right-0 top-full z-50 grid w-[760px] grid-cols-3 gap-6 rounded-b-3xl border border-stone-200 bg-[#fffaf3] p-6 shadow-2xl">{parentCategories.map((parent) => <div key={parent.slug}><Link href={`/category/${parent.slug}`} className="flex items-center gap-2 border-b border-stone-200 pb-2 text-sm font-black text-stone-950 hover:text-violet-700"><span>{parent.icon}</span>{parent.name}</Link><div className="mt-3 space-y-1.5">{allCategories.filter((category) => category.parentSlug === parent.slug).map((category) => <Link key={category.slug} href={`/category/${category.slug}`} className="block text-[11px] font-semibold text-stone-500 hover:text-violet-700">{category.icon} {category.name}</Link>)}</div></div>)}</div>}</div><Link href="/virtual-tryon" className="inline-flex items-center gap-1 rounded-full bg-violet-700 px-4 py-2 text-xs font-black text-white shadow-md shadow-violet-200 hover:bg-violet-800"><Sparkles className="size-3.5" /> پرو آنلاین سایز</Link><Link href="/blog" className="px-4 py-3 text-xs font-black text-stone-900 hover:text-violet-700">مجله استایل</Link><Link href="/contact" className="mr-auto px-4 py-3 text-xs font-bold text-stone-500 hover:text-violet-700">تماس با ما</Link></div></nav>
      {menuOpen && <div className="border-t border-stone-200 bg-[#fffaf3] p-4 shadow-xl lg:hidden"><div className="grid gap-2 sm:grid-cols-2"><Link href="/virtual-tryon" onClick={closeMobile} className="col-span-full rounded-2xl bg-gradient-to-l from-violet-700 to-fuchsia-600 p-4 text-center text-sm font-black text-white shadow-lg"><span className="flex items-center justify-center gap-2"><Sparkles className="size-4" /> پرو آنلاین سایز</span><span className="mt-1 block text-[10px] font-semibold text-violet-100">انتخاب سایز دقیق کودک در چند مرحله</span></Link><Link href="/shop" onClick={closeMobile} className="rounded-xl bg-stone-950 p-3 text-center text-xs font-black text-white">همه محصولات</Link><Link href="/account?mode=register" onClick={closeMobile} className="rounded-xl bg-amber-300 p-3 text-center text-xs font-black text-stone-950">عضویت رایگان</Link></div><div className="mt-5 grid gap-4">{parentCategories.map((parent) => <div key={parent.slug}><Link href={`/category/${parent.slug}`} onClick={closeMobile} className="block border-b border-stone-200 pb-2 text-sm font-black">{parent.icon} {parent.name}</Link><div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-2">{allCategories.filter((category) => category.parentSlug === parent.slug).map((category) => <Link key={category.slug} href={`/category/${category.slug}`} onClick={closeMobile} className="text-[11px] font-semibold text-stone-500">{category.icon} {category.name}</Link>)}</div></div>)}</div></div>}
    </header>
  );
}
