...
export default function ProductAngleMediaManager({ value, onChange }: Props) {
  ...
  return <div dir="rtl" className="rounded-2xl border border-amber-200 bg-amber-50/60 p-4">
    <div className="flex items-start gap-2"><Sparkles className="mt-0.5 size-5 text-amber-700" /><div><h4 className="text-xs font-black text-amber-950">رسانهٔ حرفه‌ای محصول و پرو آنلاین</h4><p className="mt-1 text-[10px] leading-5 text-amber-900">تصویر ابتدا سریع ذخیره می‌شود؛ بهینه‌سازی AI در پس‌زمینه انجام می‌شود و در صورت خطا، تصویر اصلی باقی می‌ماند.</p></div></div>
    <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">{angles.map((angle) => { const item = value[angle]; return <label key={angle} className="relative cursor-pointer overflow-hidden rounded-xl border border-amber-200 bg-white p-2 text-center hover:border-amber-500">{item?.url ? <img src={item.url} alt={item.alt || angleLabels[angle]} className="aspect-square w-full rounded-lg object-cover" /> : <span className="flex aspect-square flex-col items-center justify-center text-amber-600"><UploadCloud className="size-6" /><span className="mt-1 text-[10px] font-bold">انتخاب عکس</span></span>}<span className="mt-2 block text-[10px] font-black text-stone-800">{angleLabels[angle]}</span>{item?.isAiOptimized && <span className="absolute left-1 top-1 rounded-full bg-emerald-600 p-1 text-white"><Check className="size-3" /></span>}{busy === angle && <span className="absolute inset-0 grid place-items-center bg-white/75"><Loader2 className="size-6 animate-spin text-amber-700" /></span>}<input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={(event) => void upload(angle, event)} /></label>; })}</div>
    {message && <p className="mt-3 text-[11px] font-bold text-amber-900">{message}</p>}
  </div>;
}
