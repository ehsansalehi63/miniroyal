    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleAdd} className="space-y-4 rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <label className="text-xs font-bold text-stone-700">نام دسته *<input required value={name} onChange={(e) => setName(e.target.value)} placeholder="مثال: اکسسوری کودک" className="mt-1 w-full rounded-xl border border-stone-200 p-2.5 font-normal outline-none focus:border-amber-500" /></label>
          <label className="text-xs font-bold text-stone-700">اسلاگ URL *<input required value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="accessory" className="mt-1 w-full rounded-xl border border-stone-200 p-2.5 font-normal outline-none focus:border-amber-500" /></label>
          <label className="text-xs font-bold text-stone-700">دسته والد<select value={parentId ?? ""} onChange={(e) => setParentId(e.target.value ? Number(e.target.value) : null)} className="mt-1 w-full rounded-xl border border-stone-200 bg-white p-2.5 font-normal outline-none focus:border-amber-500"><option value="">دسته اصلی</option>{parentCategories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></label>
          <label className="text-xs font-bold text-stone-700">آیکون<input value={icon} onChange={(e) => setIcon(e.target.value)} className="mt-1 w-full rounded-xl border border-stone-200 p-2.5 font-normal outline-none focus:border-amber-500" /></label>
        </div>
        {message && <p className="rounded-xl bg-amber-50 p-3 text-xs font-bold text-amber-800">{message}</p>}
        <button type="submit" className="inline-flex items-center gap-2 rounded-2xl bg-stone-950 px-6 py-2.5 text-xs font-black text-white shadow-md hover:bg-stone-800 transition"><Plus className="size-4" />ثبت دسته‌بندی جدید</button>
      </form>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{categories.map((category) => <div key={category.id} className="flex items-center gap-4 rounded-3xl border border-stone-200 bg-white p-5 shadow-sm"><span className="grid size-12 place-items-center rounded-2xl bg-amber-50 text-2xl">{category.icon || "👕"}</span><div><h3 className="text-sm font-bold text-stone-900">{category.name}</h3><p className="text-xs text-stone-500 font-mono">/category/{category.slug}</p>{category.parentId && <p className="mt-1 text-[10px] text-amber-700 font-bold">زیرمجموعه: {categories.find((item) => item.id === category.parentId)?.name || "دسته والد"}</p>}</div></div>)}</div>
