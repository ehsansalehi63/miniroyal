...
| `TRYON_USE_VISION_PROMPT` | اجرای تحلیل اضافه Dahl/OpenRouter قبل از تولید | پیش‌فرض `false` برای جلوگیری از 504 |
...
| `GEMINI_API_KEY` | کلید رایگان AI Studio (بدون کارت، ~۵۰۰ پرو/روز)؛ همیشه اول امتحان می‌شود | پرو آنلاین |
| `GEMINI_API_KEYS` | چند کلید Gemini با comma برای چرخش روی سقف مصرف (اختیاری) | پرو آنلاین |
| `GEMINI_IMAGE_MODEL` | مدل Gemini (اختیاری؛ پیش‌فرض `gemini-2.5-flash-image`) | پرو آنلاین |
